...
// - K-panel blocking, pack A once per panel (reused across N)
// - Correct accumulation across K-panels
// - Stream only on final K-panel (full tiles)
// - Masked tails, templated UNROLL, pointer-bump addressing, tuned prefetch
#include <immintrin.h>
#include <algorithm>
#include <cstdint>
#include <cstring>
...
// one-shot UNROLL picker (fixed C buffer size & indexing)
static std::atomic<int> g_unroll_idx{-1}; // -1:not picked
static inline int pick_unroll_once_idx(int Kc_for_test){
  ...
  const int test_MT = 6;          // tiles in M (6×8 rows)
  const int test_NT = 2;          // tiles in N (2×48 cols)
  const int ldc     = test_NT*NR; // 96

  float* Ap = aligned_alloc64((size_t)test_MT * Kc_for_test * MR);
  float* Bp = aligned_alloc64((size_t)test_NT * Kc_for_test * NR);
  float* C  = aligned_alloc64((size_t)test_MT * MR * ldc);  // FIX: *MR rows per tile
  std::memset(Ap, 0, sizeof(float)*(size_t)test_MT*Kc_for_test*MR);
  std::memset(Bp, 0, sizeof(float)*(size_t)test_NT*Kc_for_test*NR);
  std::memset(C , 0, sizeof(float)*(size_t)test_MT*MR*ldc); // FIX: include MR
  ...
  auto bench = [&](int idx){
    ...
    for(int tm=0; tm<test_MT; ++tm){
      ...
      for(int tn=0; tn<test_NT; ++tn){
        ...
        float* C_t = C + (size_t)tm*MR*ldc + tn*NR;           // FIX: base offset *MR*ldc
...
#pragma omp parallel for schedule(static)
  ...
  if(beta==0.0f){
    ...
    float* A_pack = aligned_alloc64((size_t)MT * (size_t)KC * MR);

#pragma omp parallel
    {
      float* Bp = aligned_alloc64((size_t)KC*NR);

      for(int k0=0; k0<K; k0+=KC){
        const int Kc = std::min(KC, K - k0);
        const bool is_last_panel = (k0 + Kc >= K);

        // pack A(:, k0:k0+Kc) in parallel over MR tiles
#pragma omp for schedule(static)
        for(int tm=0; tm<MT; ++tm){
          const int r0 = tm*MR;
          const int mr_eff = std::min(MR, M - r0);
          const float* A_src = A + (size_t)r0*ldA + k0;
          float*       A_dst = A_pack + (size_t)tm*Kc*MR;
          pack_A_tile_mrK(A_src, ldA, mr_eff, Kc, alpha, A_dst);
        }
#pragma omp barrier

        const int NT_total = (N + NR - 1) / NR;

        // compute in parallel over NR tiles across N
#pragma omp for schedule(static)
        for(int tn=0; tn<NT_total; ++tn){
          const int j0 = tn*NR;
          const int nr_eff = std::min(NR, N - j0);
          if(nr_eff <= 0) continue;

          pack_B_tile_Knr(B + (size_t)k0*ldB + j0, ldB, Kc, nr_eff, Bp);

          for(int tm=0; tm<MT; ++tm){
            const int r_off   = tm*MR;
            const int mr_here = std::min(MR, M - r_off);
            const float* Ap_tile = A_pack + (size_t)tm*Kc*MR;
            float* C_tile = C + (size_t)r_off*ldC + j0;

            if(k0==0){
              const bool stream_ok = (is_last_panel && mr_here==MR && nr_eff==NR && aligned64(C_tile));
              micro_overwrite(Ap_tile, Bp, C_tile, ldC, Kc, mr_here, nr_eff, stream_ok);
            }else{
              micro_accum(Ap_tile, Bp, C_tile, ldC, Kc, mr_here, nr_eff);
            }
          }
        } // end tn
  ...
  // beta!=0: simple reference path (unchanged)
#pragma omp parallel for schedule(static)
  for(int i=0;i<M;++i){
    for(int j=0;j<N;++j){
      float acc = 0.f;
